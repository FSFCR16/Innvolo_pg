...
### 2026-07-06 — Fix crasheo de PC + "0 productos": caché ISR (A) + imágenes desde Vercel (B) 🟡
- **Diagnóstico:** las rutas de catálogo eran `force-dynamic` → consultaban Woo (PC local vía ngrok) **en cada visita**, y las imágenes de producto/categoría se servían desde la PC (`normalizarWpUrl` → `WC_BASE_URL`). Cada visita (PC o celular) disparaba decenas de peticiones a la PC → saturación/crasheo. Cuando el fetch tardaba +10s (`AbortSignal.timeout`) devolvía `null` → **"0 productos"**; al recargar, respondía y aparecían. El celular no se traba porque todo el trabajo (PHP/MySQL/imágenes) ocurre en la PC.
- **A — ISR por ruta:** en los 4 `page.jsx` de `catalogo/` se cambió `export const dynamic = "force-dynamic"` → `export const revalidate = 300`; en `lib/woocommerce/woo.js` el `fetch` ahora lleva `next: { revalidate: 300 }`. Resultado: cada endpoint pega a la PC **como mucho 1 vez cada 5 min** (no por visita); ISR sirve la última versión buena aunque Woo caiga (mata el "0 productos"). Verificado en build: `/catalogo` pasó de `ƒ` a **`○` con Revalidate 5m** (prueba de que el fetch quedó cacheable pese al `signal`); las rutas con params siguen `ƒ` pero se cachean on-demand 5 min.
- **B — imágenes desde Vercel (fuera de la PC):** se generaron **36 `.webp` ~800px (1.2 MB total, antes 156 MB de PNG)** en `public/productos/<idProducto>.webp` con sharp, mapeando `woo_fase0_idmap.json` (fila→id Woo) × `woo_fase0b_manifest.json` (fila→PNG). Script: `scratchpad/optimizar_imagenes.mjs`. Las queries (`queries/productos.js` y `queries/categorias.js`) ya **no** usan `normalizarWpUrl(img.src)` → devuelven `/productos/<id>.webp`; las 6 categorías top-level mapean a `public/categorias/*.jpeg` por nombre. Resultado: **cero peticiones de imagen a la PC**.
- **Sin tocar:** DB de Woo, `.glb`, visor 3D, `.glb` originales. `normalizarWpUrl` queda definida en `urls.js` pero ya sin uso. Build limpio (11/11). **Rollback:** `git checkout` de los 4 `page.jsx` + `woo.js` + `queries/productos.js` + `queries/categorias.js`, y borrar `public/productos/`.
- **Nota:** sigue necesitándose PC+LocalWP+ngrok encendidos para que el catálogo tenga datos frescos (solo 1 vez cada 5 min ahora). Solución de fondo pendiente = mover WP a hosting público (opción C, descartada por ahora). Mejora opcional futura: `generateStaticParams` para pre-generar las product pages en build.
